...
import { Calendar, FileText, Clock, AlertCircle, CheckCircle, XCircle, Plus, X } from 'lucide-react';
import axios from 'axios';

const LeaveRequestEmployee = ({ employeeId, cinemaClusterId }) => {
  ...
  const fetchLeaveRequests = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API}/employee/${employeeId}/${cinemaClusterId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Không thể tạo đơn nghỉ phép');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCancel = async (leaveRequestId) => {
    ...
    setLoading(true);
    try {
      await axios.patch(`${API}/cancel/${leaveRequestId}`, {
        employee_id: employeeId,
        cinema_cluster_id: cinemaClusterId
      });
      ...
      <div className="max-w-6xl mx-auto space-y-5">
        ...
        <div className="bg-zinc-900 border border-red-800 rounded-xl p-5">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
            <div>
              <h1 className="text-2xl font-bold text-red-500">QUẢN LÝ NGHỈ PHÉP</h1>
            ...
            >
              <Plus className="w-5 h-5" /> TẠO ĐỚN MỚI
            </button>
          </div>
        ...
        <div className="bg-zinc-900 border border-red-800 rounded-xl overflow-hidden">
          <div className="p-4 bg-gradient-to-r from-red-950 to-black border-b border-red-800">
            <h2 className="text-lg font-bold text-red-300">LỊCH SỬ ĐƠN NGHỈ PHÉP</h2>
          </div>

          {loading ? (
            <div className="text-center py-12 text-gray-500">Đang tải...</div>
          ) : leaveRequests.length === 0 ? (
            <div className="text-center py-12 text-gray-500">Chưa có đơn nghỉ phép nào</div>
          ) : (
