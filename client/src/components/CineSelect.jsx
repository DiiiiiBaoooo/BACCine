...
const CineSelect = ({ id, onSelectCinema }) => {
  ...
  const fetchCinemas = async () => {
    try {
      ...
      console.log('CineSelect API Response:', data);
      if (data.success) {
  ...
  const handleCinemaSelect = (cinemaName) => {
    setSelected(cinemaName);
    onSelectCinema(cinemaName);
  };
              ...
              <span className="grid grid-cols-3 md:flex flex-wrap md:wrap-w-lg gap-4">
                {cinemas.map((cinema) => (
                  <button
                    ...
                    className={`flex items-center justify-center h-14 w-32 rounded cursor-pointer ${
                      selected === cinema.name ? 'bg-primary text-white' : 'border border-primary/70'
                    }`}
                  >
